...
forge create --rpc-url  $ANVIL_SOURCE_CHAIN_RPC_URL --private-key $ANVIL_PRIVATE_KEY  "$CONTRACT_PATH:$CONTRACT_NAME" --broadcast --json > $ESPRESSO_APP_CONTRACTS_DATA_FILE_SOURCE
forge create --rpc-url $ANVIL_DESTINATION_CHAIN_RPC_URL --private-key $ANVIL_PRIVATE_KEY  "$CONTRACT_PATH:$CONTRACT_NAME" --broadcast --json > $ESPRESSO_APP_CONTRACTS_DATA_FILE_DESTINATION
