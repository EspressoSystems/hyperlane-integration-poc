...
? Detected rpc url as http://localhost:8545 from JSON RPC provider, is this
correct? n
? Enter http or https rpc url: (http://localhost:8545) http://localhost:8546 TODO replace with 8549 or even better use env variable
...
Fund the `HYP_KEY` address for deployment on the source chain (Caff node)

```bash
> cast send  $HYP_ADDRESS --value 1ether --private-key $CAFF_NODE_DEV_PRIV_KEY --rpc-url $SOURCE_CHAIN_RPC_URL
```
...
```bash
> cast send  $VALIDATOR_ADDRESS --value 1ether --private-key $CAFF_NODE_DEV_PRIV_KEY --rpc-url $SOURCE_CHAIN_RPC_URL
```
...
```bash
> cast send  $RELAYER_ADDRESS --value 1ether --private-key $HYP_KEY --rpc-url $DESTINATION_CHAIN_RPC_URL
```
